...
void do_decompression(const char *source_dir_path, const char *output_dir_path) {
    ...
    // {
    //     # pragma omp parallel critial 
    //     {
    //         get_compressed_packages_path(path, source_dir_path);    //  找到所有的.zwz文件
    //     }
        
    //     # pragma omp braier // 找个办法让critical先执行

    //     # pragma omp sectiion 
    //     {
    //         # pragma omp tasks
    //         {
    //             do_compression(tree/map, path[thread_x], outputpath_thread_x)
    //         }    
    //     }

    //     # pragma omp section 
    //     {
    //         // 根据txt文件，生成相对路径的文件夹，将所有解压在暂存区的文件放入到对应的相对路径下
    //     }
    // }
    ...
    //     # pragma omp critical 
    //     { 
    //         file_path, output_dir_path = readFile(); 
    //     } 
    //         decompress_zwz() 
    // }
